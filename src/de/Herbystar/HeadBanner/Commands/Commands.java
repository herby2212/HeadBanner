			...
			if(cmd.getName().equalsIgnoreCase("Banners")) {
				if(plugin.getConfig().getBoolean("HeadBanner.Enabled") == true) {
					if(p.hasPermission("HeadBanner.Item.Open") | p.isOp()) {
						plugin.inv = p.getPlayer().getServer().createInventory(null, plugin.getConfig().getInt("HeadBanner.InventorySize.1"), plugin.replaceString(plugin.getConfig().getString("HeadBanner.BannerInventory.Side1Title")));
						
						ItemStack B1 = new ItemStack(Material.BANNER);
						BannerMeta B1M = (BannerMeta) B1.getItemMeta();
						B1M.setDisplayName(plugin.replaceString(plugin.getConfig().getString("HeadBanner.BannerDisplayNames.1")));
						...
						EM.setDisplayName(plugin.replaceString(plugin.getConfig().getString("HeadBanner.GUI.ClearHeadItemName")));
						E.setItemMeta(EM);
						
						ItemStack C = new ItemStack(Material.SLIME_BALL);
						ItemMeta CM = C.getItemMeta();
						CM.setDisplayName(plugin.replaceString(plugin.getConfig().getString("HeadBanner.GUI.CloseItemName")));
						C.setItemMeta(CM);
						
						ItemStack N = new ItemStack(Material.MAGMA_CREAM);
						ItemMeta NM = N.getItemMeta();
						NM.setDisplayName(plugin.replaceString(plugin.getConfig().getString("HeadBanner.GUI.NextSide")));
						N.setItemMeta(NM);
						
						ItemStack R = new ItemStack(Material.ARMOR_STAND);
						ItemMeta RM = R.getItemMeta();
						RM.setDisplayName(plugin.replaceString(plugin.getConfig().getString("HeadBanner.GUI.RotateItemName")));
						R.setItemMeta(RM);
